...
app.get("/user/photos-preview/:id", async (request, response) => {
  ...
  try {
    // Fetch all photos of the user
    const photos = await Photo.find({ user_id: userId }, "_id file_name date_time comments");
    ...
    const mostCommentedPhoto = photos.reduce((max, photo) => {
      return (photo.comments?.length || 0) > (max.comments?.length || 0) ? photo : max;
    });
    ...
    console.error("Error fetching user photos preview:", error);
    return response.status(500).send("Internal server error");
  }
});
...
app.post("/photos/:photo_id/like", requireLogin, async (req, res) => {
  ...
  try {
    ...
    if (photo.likes.includes(userId)) {
      return res.status(400).send("User has already liked this photo");
    }
    ...
    return res.status(200).json({ likesCount: photo.likes.length });
  } catch (error) {
...
app.post("/photos/:photo_id/unlike", requireLogin, async (req, res) => {
  ...
  try {
    ...
    // Check if the user has not liked the photo
    if (!photo.likes.includes(userId)) {
      return res.status(400).send("User has not liked this photo");
    }
